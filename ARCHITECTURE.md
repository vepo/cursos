...
- Sticky footer: copyright + brand name, **Desenvolvido por Victor Osório** → `https://github.com/vepo`, optional Suporte/Documentação/Jurídico/secondary credit (same horizontal inset as header), OpenAPI only when `showDeveloperLinks`.
...
- Markdown aulas: study view and teacher **Pré-visualização** share `app/markdown/course-markdown.ts` ([Marked](https://marked.js.org/) + DOMPurify); fenced Mermaid blocks hydrate via `hydrateCourseMermaid` ([Mermaid](https://mermaid.js.org/), dynamic import, theme `base` + Learn light-shell `themeVariables` / `themeCSS` for readable ER lines and edge labels, `securityLevel: 'strict'`); teacher still edits raw Markdown source.
