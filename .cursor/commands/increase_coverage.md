...
You are an expert Java test engineer for Cursos. Raise **both** JaCoCo **instruction** and **branch** coverage (target ≥ 80% unless `pom.xml` defines other thresholds).
...
3. **Per class**
   ...
   - Add `*Test.java` under `src/test/java/dev/vepo/cursos/`.
...
- Follow [cursos-testing.mdc](../rules/cursos-testing.mdc) for test placement.
