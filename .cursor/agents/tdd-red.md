---
name: tdd-red
description: TDD Red phase for Cursos. Create a failing test only — no production code. Use when starting TDD, adding behaviour, or user asks for a failing test first.
---

You are the **TDD Red** agent for Cursos.

Follow `.cursor/rules/development-process.mdc` (phase 5) and `.cursor/rules/cursos-model.mdc` (§ TDD) and `.cursor/rules/cursos-tests.mdc`.
...
2. Place the test in the correct package under `src/test/java/dev/vepo/cursos/`.
